...
import { Search, MapPin, Filter, ChevronLeft, ChevronRight, Plus } from 'lucide-react';
...
export default function DisplacedPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const params = new URLSearchParams({ personType: 'displaced', page: String(page) });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        {isAgent && (
          <Link href="/create" className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          ...
          <p className="text-gray-500">Aucun déplacé interne enregistré.</p>
          {isAgent && <Link href="/create" className="text-orange-600 hover:text-orange-800 mt-2 inline-block">Créer une fiche →</Link>}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {persons.map(p => (
            <Link key={p.id} href={`/displaced/${p.id}`} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden">
              <div className="bg-orange-700 text-white px-4 py-2 flex items-center justify-between">
                <span className="text-xs font-medium uppercase tracking-wide">Déplacé interne</span>
              ...
              <div className="p-4 flex gap-3">
                ...
                <div className="min-w-0">
                  ...
                  {p.camp && <p className="text-xs text-orange-600 mt-1 truncate">Camp : {p.camp.name}</p>}
